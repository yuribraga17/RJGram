...
export const validarTokenJWT = (handler : NextApiHandler) =>
    ...
    try{
        ...
        if(!MINHA_CHAVE_JWT){
            return res.status(500).json({ erro : 'Ops: Chave JWT não informada na execução do projeto.'});
        }
    
        if(!req || !req.headers){
            return res.status(401).json({erro: 'Ops: Não foi possível validar o token de acesso.'});
        }
